...
float set_height[2]={0.0f},error_height[2]={0.0f},sum_height=0.0f;

float u=0.0f,u_roll=0.0f,u_pitch=0.0f,u_yaw=0.0f,kp=0.0f,ki=0.0f,kd=0.0f,ccr;       //pitch 1.3,1.9,0.018

float k_p[4]={1.4,1.5,1.90,0.0},k_i[4]={0.135,0.125,0.0,0.0},k_d[4]={0.24,0.29,0.8,0};

//float k_p[4]={1.4,1.5,0,0},k_i[4]={0.135,0.125,0.0,0.0},k_d[4]={0.24,0.29,0,0};

//float k_p[4]={1.4,1.5,0,0},k_i[4]={0.135,0.45,0.0,0},k_d[4]={0.24,0.25,0,0};        //stable values pitch 0.56,0.115,0.29

//float k_p[4]={0,0,2.0,0},k_i[4]={0,0,0.0,0},k_d[4]={0,0,0.0,0};  
...
float ccr_throttle=0.0f,ccr_height=0.0f;
...
char m=1;
float MagBuffer[3] = {0.0f},mag=0.0f,dir=0.0f,initdir=0.0f,yaw_dir=0.0f,yaw_dir_avg=0.0,sum_dir=0.0f;
...
int test=0;
//float vel_z=0,act_z[2]=0;

uint16_t IC3ReadValue1 = 0, IC3ReadValue2 = 0, CaptureNumber = 0;
uint32_t echo_capture = 0; 
float height[2] = {0};
unsigned int echo_flag=0,trigger_flag=0;


float offset=0,pitch_est1=0,roll_est1=0,roll_est_temp[2]={0},pitch_est_temp[2]={0},yaw_est_temp[2]={0};
...
unsigned int absCCR_1=0,prevabsCCR_1[5]={0};
...
unsigned long	usartdata=0;
unsigned int usartsend,var=0,i=0;	
	
float speed_track_error=0,c7=0.2,c8=0.4,set_height_change=0,height_change=0, lambda=1;	
//////////////0.3 0.5 1

float prev_ccr_yaw[order]={0.0f},fltr_ccr_yaw=0;
int med_i=0,med_flag=0;

int back_flag=0,yaw_flag=0;	
...
int main(void)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2 | RCC_APB1Periph_TIM3 | RCC_APB1Periph_TIM4 |RCC_APB1Periph_TIM6 | RCC_APB1Periph_TIM7, ENABLE);
	...
	//			ccr_throttle=750;	
		  ...
		  TIM3->CCR1=0;     //front
			TIM3->CCR4=0;     //back
 			TIM3->CCR2=0;     //left
			TIM3->CCR3=0;     //right
			CCR1_Val=CCR2_Val=CCR3_Val=CCR4_Val=ccr_front_motor=ccr_back_motor=ccr_right_motor=ccr_left_motor=0;
			sum_roll=sum_pitch=sum_yaw=ccr_roll=ccr_pitch=fltr_ccr_yaw=ccr_height=0; 		
			//k_p[3]=0;
 			...
 			if(pidupdate==1)
			{
				...
				roll_est_temp[1] = (int) (roll_est*10);								
				roll_est_temp[1] = (roll_est_temp[1]/10);
				pitch_est_temp[1] = (int) (pitch_est*10);								
				pitch_est_temp[1] = (pitch_est_temp[1]/10);
				yaw_est_temp[1] = (int) (yaw_dir_avg*10);								
				yaw_est_temp[1] = (yaw_est_temp[1]/10);
				
					
				
			if(ccr_throttle>20 && offset==1500)
			{
				
//				
//				roll_est_temp = (int) (roll_est*10);								
//				roll_est_temp = (roll_est_temp/10);
//				pitch_est_temp = (int) (pitch_est*10);								
//				pitch_est_temp = (pitch_est_temp/10);
//				yaw_est_temp = (int) (yaw_dir*10);								
//				yaw_est_temp = (yaw_est_temp/10);
		
////				 	
				
				error_roll[0]= (error_roll[1]);
				error_roll[1]=(roll_est_temp[1]-set_roll-roll_est1);
				if(error_roll[1]>0.6)
				...
				ccr_roll=k_p[0]*error_roll[1] + k_i[0]*sum_roll + k_d[0]*(roll_est_temp[1]-roll_est_temp[0])/t;
				roll_est_temp[0] = roll_est_temp[1];
				...
				error_pitch[1]=(pitch_est_temp[1]-set_pitch-pitch_est1);
				if(error_pitch[1]>0.6)
				...
				ccr_pitch=k_p[1]*error_pitch[1] + k_i[1]*sum_pitch + k_d[1]*(pitch_est_temp[1]-pitch_est_temp[0])/t;
				pitch_est_temp[0] = pitch_est_temp[1];
...
//				if (yaw_flag==4)
				...
				error_yaw[1]=(yaw_est_temp[1]-set_yaw);
				if(error_yaw[1]>1)
				...
				ccr_yaw=k_p[2]*error_yaw[1] + k_d[2]*(yaw_est_temp[1]-yaw_est_temp[0])/t;
				
				//ccr_yaw=k_p[2]*error_yaw[1] + 0*(error_yaw[1]-error_yaw[0])/t;
				
				yaw_est_temp[0] = yaw_est_temp[1];
				
				//fltr_ccr_yaw = ccr_yaw;
				fltr_ccr_yaw=0.1*ccr_yaw+0.9*fltr_ccr_yaw;
				}
	
//				prev_ccr_yaw[med_i]=ccr_yaw;
//				med_i++;
//				if(med_i==order)
//				{
//					med_i=0;
...
//				
				...
				if(set_height[1]>=50&& back_flag==1)
				{
				back_flag=0;	
				error_height[0]=error_height[1];
				error_height[1]=(set_height[1]-height[1])/100;   /////100 divide for backstep
				if(error_height[1]>0.05)
				sum_height=sum_height+(error_height[1]*(t*10));
				
					set_height_change=((set_height[1]/100)-set_height[0])/(t*10);
					set_height[0]=set_height[1]/100;
					height_change=((height[1]/100)-height[0])/(t*10);
					height[0]=height[1]/100;						
					
					speed_track_error= (c7*error_height[1]+set_height_change+lambda*sum_height-height_change);					
// back-stepping for the height
					ccr_height= (1.2/(cos(pitch_est/kangle)*cos(roll_est/kangle)))*(9.8+(1-(c7*c7)+lambda)*error_height[1]+(c7+c8)*speed_track_error-c7*lambda*sum_height+(height_change/(t*10)));					
				
				
				//ccr_height=k_p[3]*error_height[1] + k_i[3]*sum_height + k_d[3]*(error_height[1]-error_height[0])/t;
				}
				...
				ccr_front_motor=ccr_throttle+ccr_height+ccr_pitch-fltr_ccr_yaw;
					if(ccr_front_motor>850)
						ccr_front_motor=850;
					if(ccr_front_motor<520)
						ccr_front_motor=520;
				
				ccr_back_motor=ccr_throttle+ccr_height-ccr_pitch-fltr_ccr_yaw;
					if(ccr_back_motor>850)
						ccr_back_motor=850;
					if(ccr_back_motor<520)
						ccr_back_motor=520;
					
				ccr_left_motor=ccr_throttle+ccr_height+ccr_roll+fltr_ccr_yaw;
					if(ccr_left_motor>850)
						ccr_left_motor=850;
					if(ccr_left_motor<520)
						ccr_left_motor=520;
					
				ccr_right_motor=ccr_throttle+ccr_height-ccr_roll+fltr_ccr_yaw;
					if(ccr_right_motor>850)
						ccr_right_motor=850;
					if(ccr_right_motor<520)
						ccr_right_motor=520;
				}
...
// 				
					...
					//	yaw_dir = angle * asin(-sin((dir)/angle) * cos(initdir/angle) + cos((dir)/angle)*sin(initdir/angle));
							...
							if (sin_angle<0 && cos_angle >0)
								yaw_dir=-yaw_dir;
							yaw_dir_avg=0.8*yaw_dir_avg+0.2*yaw_dir;
				}
...
roll_P[2][2]= - roll_K[2][0]*roll_P02_temp - roll_K[2][1]*roll_P12_temp + roll_P[2][2];	
		...
		if (offset<1500)		
		{  
		   pitch_est1=pitch_est_temp[1];
			 roll_est1=roll_est_temp[1];
				offset++;
		}
...
//timer 3 pwm outputs
  ...
  NVIC_Init(&NVIC_InitStructure);
	...
	//////timer 1 ip capture echo ultrasonic//////////////////////
...
/////////////tim15 trigger pin ultrasonic//////////////////
  ...
  TIM_TimeBaseInit(TIM15, &TIM_TimeBaseStructure);
	...
	TIM_ITConfig(TIM15, TIM_IT_Update, ENABLE); //i have tested in debug mode and the code just stops executing after this line
		
	NVIC_InitStructure.NVIC_IRQChannel = TIM1_BRK_TIM15_IRQn; //TIM4_IRQn
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;
...
//				if (counts>36000)
//						counts=18000;		
//				CCR=(float)(counts-1350)*100.0/2700.0;      //5 degrees till 1600
//				//k_i[0]=0.5+CCR_3;
...
//				
//				prevabsCCR_1[0] = prevabsCCR_1[1] ;
//				prevabsCCR_1[1] = prevabsCCR_1[2] ;
//				prevabsCCR_1[2] = prevabsCCR_1[3] ;
//				prevabsCCR_1[3] = prevabsCCR_1[4] ;
//				prevabsCCR_1[4] = CCR_1;									
//									
//				
//				CCR_1 = (unsigned int)(prevabsCCR_1[4]+prevabsCCR_1[3]+prevabsCCR_1[2]+prevabsCCR_1[1] +prevabsCCR_1[0])/5;
//				
//				CCR_1 = (unsigned int)(CCR_1);
...
//				
				...
				set_height[1] = (int) (absCCR_1*10); 
				
				if(set_height[1]<=40)
					set_height[1]=0;

				if(set_height[1]>40 && set_height[1]<=70)
					set_height[1]=70;				
				...
				else //if(!((prevabsCCR_1==17)||(prevabsCCR_1==16)||(prevabsCCR_1==15)||(prevabsCCR_1==18)))
				{	
				//		kill_sw=1;
					kill_sw = 0;
					set_height[1]=0;
...
{ 
   if(!(TIM_GetITStatus(TIM1, TIM_IT_CC2)) == RESET) 
  {
    ...
    else if(CaptureNumber == 1)
    {
      ...
      {
        echo_capture = (IC3ReadValue2 - IC3ReadValue1); 
      }
      else if (IC3ReadValue2 < IC3ReadValue1)
      {
        echo_capture = ((0xFFFF - IC3ReadValue1) + IC3ReadValue2); 
      }
      else
      {
        echo_capture = 0;
      ...
      height[1] = (int)(echo_capture/29/2);
...
//{
//	
//								usartdata =(roll_est + 100)*100;
//							//usartdata = (pitch_est+1000)*100;
//							//usartdata = (1000+1000)*1;
//							times=0;
//						//USART_SendData(USART3, 48);
//							while(times<5)							
//							{ 
//								a_uart= pow(10,4-times);
//								usartsend = usartdata/a_uart;
//								USART_SendData(USART3, usartsend+48);
//								usartdata = usartdata%a_uart;
//								times++;  
//								int i2=1000;                    //////475
//								while(i2--);
//							
...
//	    {
//	      if (prev_ccr_yaw[d] > prev_ccr_yaw[d+1]) /* For decreasing order use < */
//	      {
//	        float swap = prev_ccr_yaw[d];
//	        prev_ccr_yaw[d]   = prev_ccr_yaw[d+1];
//	        prev_ccr_yaw[d+1] = swap;
//	      }
//	    }
//	  }

//	fltr_ccr_yaw = prev_ccr_yaw[order_val/2];
//}
